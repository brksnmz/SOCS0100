...
- Using the element inspector, we see that the table is a dynamic table
  - the data of interest are in table rows **`<tr>`** of class 
...
``` r
main <- remote_driver$findElements(using = "css", value = ".BNYQd")
...
``` r
webElem <- remote_driver$findElement("css", ".BNYQd")
webElem$sendKeysToElement(list(key = "end"))
...
billionaires <- doc %>%
  html_element("table") %>%       # or html_element(xpath = ".//table")
  html_table(fill = TRUE)

```

## Inspecting data from the table {.smaller}
...
billionaires <- billionaires %>% mutate_all(~ ifelse(. == "", NA, .))

# Clean and convert 'money' to numeric
billionaires$money <- as.numeric(gsub("\\$|B", "", billionaires$money))
```
...
total_money_by_country <- aggregate(money ~ country, data = billionaires, sum)
